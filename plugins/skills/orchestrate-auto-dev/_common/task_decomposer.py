...
def parse_story_tasks(story_file: Path) -> List[Task]:
    ...
    """
    logger.debug(f"Parsing tasks from {story_file.name}")
    ...
    if bmad_match:
        logger.info(f"Detected BMAD format in {story_file.name}")
    ...
    if not tasks_match:
        logger.warning(f"No tasks section found in {story_file.name}")
        return []

    logger.info(f"Detected flat format in {story_file.name}")
...
def _parse_bmad_format(content: str, start_pos: int) -> List[Task]:
    ...
    logger.debug(f"Found {len(task_matches)} task headers in BMAD format")
    ...
    for i, match in enumerate(task_matches):
        ...
        logger.debug(f"Task {task_num}: {len(subtasks)} subtasks, complete={is_complete}")
    ...
    logger.info(f"Parsed {len(tasks)} tasks from BMAD format")
    return tasks
...
def _parse_flat_format(content: str, start_pos: int) -> List[Task]:
    ...
    for i, match in enumerate(task_matches, 1):
        ...
        logger.debug(f"Task {i}: {len(subtasks)} subtasks, complete={is_complete}")
    ...
    logger.info(f"Parsed {len(tasks)} tasks from flat format")
    return tasks
...
def should_decompose(tasks: List[Task]) -> bool:
    ...
    should_decomp = len(incomplete_tasks) > 0
    logger.info(f"Decomposition decision: {should_decomp} ({len(incomplete_tasks)}/{len(tasks)} incomplete)")

    # Return True if any incomplete tasks exist
    return should_decomp
...
def get_incomplete_tasks(tasks: List[Task]) -> List[Task]:
    ...
    logger.debug(f"Found {len(incomplete)} incomplete tasks out of {len(tasks)} total")
    return incomplete
...
def create_task_chunks(tasks: List[Task], chunk_size: int = 4) -> List[List[Task]]:
    ...
    logger.info(f"Created {len(chunks)} chunks from {len(tasks)} tasks (chunk size: {chunk_size})")
    return chunks
