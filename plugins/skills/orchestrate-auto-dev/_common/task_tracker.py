...
class TaskTrackerManager:
    ...
    def initialize(self, story_id: str, story_file: str) -> Path:
        ...
        """
        logger.info(f"Initializing task tracker for story {story_id}")
        ...
        self._save()
        logger.debug(f"Task tracker saved to {self.tracker_file}")
    ...
    def load(self, story_id: str) -> bool:
        ...
        """
        logger.debug(f"Loading task tracker for story {story_id}")
        self.tracker_file = self.temp_dir / f"tasks_{story_id}.yaml"

        if not self.tracker_file.exists():
            logger.warning(f"Task tracker file not found: {self.tracker_file}")
            return False
        ...
        logger.info(
            f"Loaded task tracker: {self.tracker.total_tasks} total tasks, "
            f"{self.tracker.completed_tasks} completed"
        )
        return True
    ...
    def add_task(
        ...
        logger.info(f"Adding task {task_id}: {description}")
    ...
    def update_status(
        ...
        for task in self.tracker.sub_tasks:
            if task['id'] == task_id:
                ...
                logger.info(f"Task {task_id}: {old_status} → {status}" +
                           (f" ({duration_seconds:.1f}s)" if duration_seconds else ""))
        ...
        self._save()
        logger.debug(f"Task tracker updated: {self.tracker.completed_tasks}/{self.tracker.total_tasks} completed")
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'failed': failed,
            'success_rate': (
                f"{completed / self.tracker.total_tasks * 100:.1f}%"
                if self.tracker.total_tasks > 0 else "0%"
            ),
        }
