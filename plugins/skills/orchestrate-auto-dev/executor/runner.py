...
class AutoDevRunner:
    ...
    # Stage requirement mappings for different story types
    STAGE_REQUIREMENTS = {
        "infrastructure": {
            "required": ["create-story", "validate", "develop", "lint", "typecheck",
                        "code-review", "git-commit", "git-push", "pr-create", "pr-checks", "pr-merge"],
            "skip": ["test-case-generation", "unit-test", "deploy-local", "generate-scripts",
                    "smoke-tests", "deploy", "critical-sqa", "full-sqa"],
            "reason": "Infrastructure/setup stories don't need testing or deployment stages"
        },
        "setup": {
            "required": ["create-story", "validate", "develop", "lint", "typecheck",
                        "code-review", "git-commit", "git-push", "pr-create", "pr-checks", "pr-merge"],
            "skip": ["test-case-generation", "unit-test", "deploy-local", "generate-scripts",
                    "smoke-tests", "deploy", "critical-sqa", "full-sqa"],
            "reason": "Setup/configuration stories don't need testing or deployment stages"
        },
        "documentation": {
            "required": ["create-story", "validate", "develop", "lint",
                        "code-review", "git-commit", "git-push", "pr-create", "pr-checks", "pr-merge"],
            "skip": ["test-case-generation", "typecheck", "unit-test", "deploy-local",
                    "generate-scripts", "smoke-tests", "deploy", "critical-sqa", "full-sqa"],
            "reason": "Documentation stories only need basic quality checks"
        },
        "refactor": {
            "required": ["create-story", "validate", "develop", "test-case-generation", "lint",
                        "typecheck", "unit-test", "code-review", "git-commit", "git-push",
                        "pr-create", "pr-checks", "pr-merge"],
            "skip": ["deploy-local", "generate-scripts", "smoke-tests", "deploy", "critical-sqa", "full-sqa"],
            "reason": "Refactoring requires tests but typically not deployment stages"
        },
        "feature": {
            "required": ["create-story", "validate", "develop", "test-case-generation", "lint",
                        "typecheck", "unit-test", "deploy-local", "generate-scripts", "smoke-tests",
                        "code-review", "git-commit", "git-push", "pr-create", "pr-checks",
                        "deploy", "critical-sqa", "pr-merge", "full-sqa"],
            "skip": [],
            "reason": "Feature stories need all stages including testing and deployment"
        },
        "bugfix": {
            "required": ["create-story", "validate", "develop", "test-case-generation", "lint",
                        "typecheck", "unit-test", "code-review", "git-commit", "git-push",
                        "pr-create", "pr-checks", "pr-merge"],
            "skip": ["deploy-local", "generate-scripts", "smoke-tests", "deploy", "critical-sqa", "full-sqa"],
            "reason": "Bug fixes need tests but typically not all deployment stages"
        },
        "default": {
            "required": ["create-story", "validate", "develop", "test-case-generation", "lint",
                        "typecheck", "unit-test", "code-review", "git-commit", "git-push",
                        "pr-create", "pr-checks", "pr-merge"],
            "skip": [],
            "reason": "Default: run core stages, optional stages depend on config"
        }
    }

    def _determine_story_type(self, story_id: str, story_file: str) -> str:
        """
        Analyze story content to determine its type.
        ...
        Returns:
            Story type: "infrastructure", "setup", "documentation", "feature",
                       "bugfix", "refactor", or "default"
        """
        try:
            content = safe_read(Path(story_file))
            if not content:
                log(f"  ⚠ Could not read story file, using default type")
                return "default"

            content_lower = content.lower()

            # Check for infrastructure keywords (check BEFORE setup for priority)
            infra_keywords = ["infrastructure", "ci/cd", "cicd", "devops", "docker",
                            "kubernetes", "k8s", "cloud setup", "aws setup", "gcp setup",
                            "terraform", "ansible", "jenkins", "github actions"]
            if any(keyword in content_lower for keyword in infra_keywords):
                log(f"  📋 Story type detected: INFRASTRUCTURE (no testing/deployment needed)")
                return "infrastructure"

            # Check for setup/configuration keywords
            setup_keywords = ["setup", "configuration", "configure", "config file",
                            "environment variable", "env var", "initial setup",
                            "project setup", "workspace setup"]
            if any(keyword in content_lower for keyword in setup_keywords):
                log(f"  📋 Story type detected: SETUP (no testing/deployment needed)")
                return "setup"

            # Check for documentation keywords
            doc_keywords = ["documentation", "readme", "docs update", "document",
                          "user guide", "api docs", "add comments", "update docs"]
            if any(keyword in content_lower for keyword in doc_keywords):
                log(f"  📋 Story type detected: DOCUMENTATION (basic checks only)")
                return "documentation"

            # Check for refactoring keywords
            refactor_keywords = ["refactor", "restructure", "code cleanup", "improve code",
                               "optimize", "simplify", "reorganize"]
            if any(keyword in content_lower for keyword in refactor_keywords):
                log(f"  📋 Story type detected: REFACTOR (tests needed, deployment optional)")
                return "refactor"

            # Check for bug fix keywords
            bugfix_keywords = ["fix bug", "bug fix", "resolve issue", "fix error",
                             "fix crash", "fix problem", "hotfix"]
            if any(keyword in content_lower for keyword in bugfix_keywords):
                log(f"  📋 Story type detected: BUGFIX (tests needed, deployment optional)")
                return "bugfix"

            # Check for feature keywords
            feature_keywords = ["new feature", "add feature", "implement", "enhancement",
                              "user story", "functionality", "capability", "create new",
                              "build", "develop feature"]
            # Also check for "add" + common feature words
            feature_indicators = ["add user", "add page", "add component", "add endpoint",
                                "add api", "add dashboard", "add form", "add button"]
            if any(keyword in content_lower for keyword in feature_keywords):
                log(f"  📋 Story type detected: FEATURE (full pipeline)")
                return "feature"
            ...
            # Default
            log(f"  📋 Story type: DEFAULT (core stages + optional per config)")
            return "default"

        except Exception as e:
            log(f"  ⚠ Error determining story type: {e}, using default")
            return "default"

    def _should_run_stage_for_story(self, stage_name: str, story_type: str) -> Tuple[bool, Optional[str]]:
        """
        Determine if a stage should run based on story type.

        Args:
            stage_name: Name of the stage
            story_type: Type of story (from _determine_story_type)

        Returns:
            Tuple of (should_run: bool, skip_reason: Optional[str])
        """
        # Get stage requirements for this story type
        requirements = self.STAGE_REQUIREMENTS.get(story_type, self.STAGE_REQUIREMENTS["default"])

        # Check if stage is in skip list
        if stage_name in requirements["skip"]:
            return False, requirements["reason"]

        # For default type, let config decide for optional stages
        if story_type == "default":
            return True, None

        # For other types, check if stage is required
        if stage_name in requirements["required"]:
            return True, None

        # Stage not in required list, skip it
        return False, requirements["reason"]
    ...
    def run(self, story_id: Optional[str] = None) -> PipelineResult:
        ...
        try:
            ...
            if not story_id:
                ...
                if task_output:
                    ...
                    if story_id:
                        ...
                        # Update status: story created → "drafted"
            ...
            # CRITICAL: Story MUST be validated before development starts
            if not passed and not self._is_disabled("validate"):
                result.error = "Story validation failed - aborting pipeline"
                log("  ❌ Story validation failed - pipeline cannot continue")
                log("  (Cannot develop a story that hasn't been validated)")
                return result

            # Update status: story validated → "ready-for-dev"
            if passed:
                self._update_story_status(story_id, "ready-for-dev")

            # Determine story type for intelligent stage execution
            log("\n=== Analyzing story type for stage requirements ===")
            story_type = self._determine_story_type(story_id, str(self.story_file))
            ...
            # Update status: story → in-progress
            self._update_story_status(story_id, "in-progress")

            # Start test-case-generation in parallel (spawn but don't wait yet)
            test_gen_task = None
            test_gen_logger = None  # Stage logger for test-case-generation

            # Check if test-case-generation is needed for this story type
            should_run_test_gen, skip_reason = self._should_run_stage_for_story("test-case-generation", story_type)
            test_gen_enabled = not self._is_disabled("test-case-generation") and should_run_test_gen

            if not should_run_test_gen:
                log(f"  test-case-generation: SKIPPED ({skip_reason})")
                result.stage_results["test-case-generation"] = "SKIP"

            if test_gen_enabled:
                log("  🚀 Spawning test-case-generation agent (parallel, non-blocking)...")
                try:
                    # Create stage logger for test-case-generation
                    test_gen_logger = self._get_stage_logger("test-case-generation", story_id)
                    test_gen_logger.info("="*60)
                    test_gen_logger.info(f"STAGE: test-case-generation (parallel)")
                    test_gen_logger.info("="*60)
                    ...
                    if is_valid:
                        test_gen_logger.info(f"  Pre-validation passed: {validation_msg}")
                        ...
                        if stage_config:
                            ...
                            test_gen_logger.info(f"  ✅ Test-case-generation agent spawned in background")
                            log(f"  ✅ Test-case-generation agent spawned in background")
                        else:
                            test_gen_logger.warn(f"  ⚠ Stage config not found")
                            log(f"  ⚠ test-case-generation stage config not found")
                    else:
                        test_gen_logger.warn(f"  ⚠ Pre-validation failed: {validation_msg}")
                        log(f"  ⚠ Skipping test-case-generation: {validation_msg}")
                except Exception as e:
                    if test_gen_logger:
                        test_gen_logger.error(f"  ⚠ Failed to spawn: {e}")
                    log(f"  ⚠ Failed to spawn test-case-generation: {e}")
                    test_gen_task = None
            else:
            ...
            # CRITICAL: Develop stage MUST complete successfully before continuing
            # Subsequent stages (lint, typecheck, tests) depend on developed code
            if not passed and not self._is_disabled("develop"):
                result.error = "Development failed - aborting pipeline"
                log("  ❌ Development stage failed - pipeline cannot continue")
                log("  (Subsequent stages require successfully developed code)")
                return result
            ...
            if test_gen_task:
                ...
                try:
                    test_result = self._wait_for_task(test_gen_task, "test-case-generation", stage_logger=test_gen_logger)

                    if test_result.success:
                        # Health check TDM file
                        health_passed, health_msg = self._verify_tdm_created(story_id)
                        if health_passed:
                            log(f"  ✅ Test case generation completed successfully")
                            log(f"  {health_msg}")
                            result.stage_results["test-case-generation"] = "PASS"
                        else:
                            log(f"  ⚠ Test case generation completed but health check failed: {health_msg}")
                            result.stage_results["test-case-generation"] = "FAIL"
                    else:
                        log(f"  ⚠ Test case generation failed (non-blocking)")
                        result.stage_results["test-case-generation"] = "FAIL"
                except Exception as e:
                    log(f"  ⚠ Error checking test-case-generation result: {e}")
                    result.stage_results["test-case-generation"] = "FAIL"
            ...
            should_run_typecheck, skip_reason = self._should_run_stage_for_story("typecheck", story_type)
            if not should_run_typecheck:
                log(f"  typecheck: SKIPPED ({skip_reason})")
                result.stage_results["typecheck"] = "SKIP"
            elif not self._is_disabled("typecheck"):
            ...
            should_run_unit_test, skip_reason = self._should_run_stage_for_story("unit-test", story_type)
            if not should_run_unit_test:
                log(f"  unit-test: SKIPPED ({skip_reason})")
                result.stage_results["unit-test"] = "SKIP"
            elif not self._is_disabled("unit-test"):
            ...
            should_run_deploy_local, skip_reason = self._should_run_stage_for_story("deploy-local", story_type)
            if not should_run_deploy_local:
                log(f"  deploy-local: SKIPPED ({skip_reason})")
                result.stage_results["deploy-local"] = "SKIP"
            elif not self._is_disabled("deploy-local"):
            ...
            # Step 8: Generate test scripts (optional)
            log("\n=== Step 8: Generating test scripts ===")
            should_run_generate_scripts, skip_reason = self._should_run_stage_for_story("generate-scripts", story_type)
            if not should_run_generate_scripts:
                log(f"  generate-scripts: SKIPPED ({skip_reason})")
                result.stage_results["generate-scripts"] = "SKIP"
            elif not self._is_disabled("generate-scripts"):
                passed = self._run_stage(
                    "generate-scripts",
                    story_id=story_id,
                    story_file=str(self.story_file)
                )
                result.stage_results["generate-scripts"] = "PASS" if passed else "FAIL"
                if not passed and self._should_abort("generate-scripts"):
                    result.error = "Test script generation failed"
                    return result
            else:
                log("  generate-scripts: DISABLED")
                result.stage_results["generate-scripts"] = "SKIP"

            # Step 9: Run smoke tests (optional)
            log("\n=== Step 9: Running smoke tests ===")
            should_run_smoke_tests, skip_reason = self._should_run_stage_for_story("smoke-tests", story_type)
            if not should_run_smoke_tests:
                log(f"  smoke-tests: SKIPPED ({skip_reason})")
                result.stage_results["smoke-tests"] = "SKIP"
            elif not self._is_disabled("smoke-tests"):
                passed = self._run_stage(
                    "smoke-tests",
                    story_id=story_id,
                    story_file=str(self.story_file)
                )
                result.stage_results["smoke-tests"] = "PASS" if passed else "FAIL"
                # Smoke tests are non-blocking (on_failure: continue)
            else:
                log("  smoke-tests: DISABLED")
                result.stage_results["smoke-tests"] = "SKIP"

            # Step 9.5: Code review (enhanced)
            log("\n=== Step 9.5: Running code review ===")
            ...
            # Step 13.5: Deploy to staging (optional)
            log("\n=== Step 13.5: Deploying to staging ===")
            should_run_deploy, skip_reason = self._should_run_stage_for_story("deploy", story_type)
            if not should_run_deploy:
                log(f"  deploy: SKIPPED ({skip_reason})")
                result.stage_results["deploy"] = "SKIP"
            elif not self._is_disabled("deploy"):
                passed = self._run_stage(
                    "deploy",
                    story_id=story_id,
                    story_file=str(self.story_file),
                ...
                result.stage_results["deploy"] = "PASS" if passed else "FAIL"
                if not passed and self._should_abort("deploy"):
                    result.error = "Staging deployment failed"
                    return result
            else:
                log("  deploy: DISABLED (may auto-deploy via CI/CD)")
                result.stage_results["deploy"] = "SKIP"

            # Step 13.6: Critical SQA tests (optional)
            log("\n=== Step 13.6: Running critical SQA tests ===")
            should_run_critical_sqa, skip_reason = self._should_run_stage_for_story("critical-sqa", story_type)
            if not should_run_critical_sqa:
                log(f"  critical-sqa: SKIPPED ({skip_reason})")
                result.stage_results["critical-sqa"] = "SKIP"
            elif not self._is_disabled("critical-sqa"):
                passed = self._run_stage(
                    "critical-sqa",
                    story_id=story_id,
                    story_file=str(self.story_file)
                )
                result.stage_results["critical-sqa"] = "PASS" if passed else "FAIL"
                # Critical SQA is non-blocking (on_failure: continue)
            else:
                log("  critical-sqa: DISABLED")
                result.stage_results["critical-sqa"] = "SKIP"

            # Step 14: PR merge (optional)
            if self.config.pr_settings.auto_merge:
                log("\n=== Step 14: Merging pull request ===")
                passed = self._run_stage(
                    "pr-merge",
                    story_id=story_id,
                    pr_url=self.pr_url,
                ...
                log("\n=== Step 14: PR merge skipped (auto_merge disabled) ===")
                result.stage_results["pr-merge"] = "SKIP"

            # Step 15: Full SQA verification (optional)
            log("\n=== Step 15: Running full SQA verification ===")
            should_run_full_sqa, skip_reason = self._should_run_stage_for_story("full-sqa", story_type)
            if not should_run_full_sqa:
                log(f"  full-sqa: SKIPPED ({skip_reason})")
                result.stage_results["full-sqa"] = "SKIP"
            elif not self._is_disabled("full-sqa"):
                passed = self._run_stage(
                    "full-sqa",
                    story_id=story_id,
                    story_file=str(self.story_file),
                ...
                result.stage_results["full-sqa"] = "PASS" if passed else "FAIL"
                # Full SQA is non-blocking (on_failure: continue)
            else:
                log("  full-sqa: DISABLED")
                result.stage_results["full-sqa"] = "SKIP"
    ...
    def _run_create_story_stage(self, story_id: Optional[str] = None) -> Tuple[bool, Optional[str]]:
        ...
        stage_config = self.config.stages.get(stage_name)
        if not stage_config:
            stage_logger.warn(f"  ⚠️  No config for {stage_name}, skipping")
            return True, None

        if not stage_config.enabled:
            stage_logger.info(f"  ℹ️  {stage_name} is disabled, skipping")
            return True, None

        # Validate story_id if provided
        if story_id:
            stage_logger.info(f"  Validating story_id: {story_id}")
            is_valid, error_msg = validate_story_id(story_id)
            if not is_valid:
                stage_logger.error(f"  ❌ Story ID validation failed: {error_msg}")
                ...
                return False, None
            stage_logger.info(f"  ✓ Story ID validated")

        max_retries = stage_config.retry.max if stage_config.retry else 0
        stage_logger.info(f"  Retry policy: max {max_retries} retries, on_failure={stage_config.on_failure}")

        # Get relevant lessons from knowledge base
        stage_logger.info(f"  Querying knowledge base for lessons...")
        limit = self._get_knowledge_limit(stage_name)
        lessons = self.knowledge.get_lessons_for_stage(stage_name, limit=limit)
        lesson_ids = [l["id"] for l in lessons]

        if lessons:
            stage_logger.info(f"  📚 Found {len(lessons)} lesson(s) from previous runs:")
            for lesson in lessons[:3]:  # Show first 3
                stage_logger.info(f"     - {lesson.get('error_type', 'unknown')}: {lesson.get('error_pattern', '')[:60]}...")
        else:
            stage_logger.info(f"  ℹ️  No previous lessons found")
        ...
        if story_id:
            ...
            stage_logger.info(f"  Creating specific story: {story_id}")
        else:
            stage_logger.info(f"  Creating next story from epics")
        ...
        for attempt in range(max_retries + 1):
            ...
            stage_logger.info(f"  Spawning {stage_name} agent...")
            stage_logger.info(f"  Timeout: {stage_config.timeout}s")

            start_time = time.time()

            # Spawn agent
            task = self.spawner.spawn_stage(stage_config, background=True, **kwargs)
            task_result = self._wait_for_task(task, stage_name, stage_logger=stage_logger)

            elapsed = time.time() - start_time

            # Check success
            if task_result.success:
                stage_logger.info(f"  ✅ {stage_name} PASSED ({elapsed:.1f}s)")

                # Health check: Verify story file was created
                stage_logger.info(f"  Running post-creation health checks...")
                health_passed, health_msg = self._verify_story_created(task_result.output, story_id)

                if not health_passed:
                    stage_logger.warn(f"  ⚠️  Health check FAILED: {health_msg}")
                    stage_logger.info(f"  Story file may not have been created correctly")
                ...
                if attempt > 0 and first_error:
                    ...
                    stage_logger.info(f"  💡 Saved lesson: {lesson_id}")

                stage_logger.info(f"  {stage_name} completed successfully")
                return True, task_result.output

            # Agent failed
            stage_logger.error(f"  ❌ {stage_name} FAILED ({elapsed:.1f}s)")
            stage_logger.info(f"  Exit code: {task_result.exit_code}")
            ...
            if task_result.error:
                ...
                if len(task_result.error) > 300:
                    stage_logger.info(f"  (Error truncated, full error in log file)")

            # Classify error severity
            if task_result.error_severity:
                stage_logger.info(f"  Error severity: {task_result.error_severity.value}")
            ...
            if attempt < max_retries and stage_config.on_failure == "fix_and_retry":
                stage_logger.info(f"  Will retry (attempt {attempt + 2}/{max_retries + 1})...")
                continue
            elif attempt >= max_retries:
                stage_logger.info(f"  No more retries available")
            elif stage_config.on_failure == "abort":
                stage_logger.info(f"  on_failure=abort, will not retry")
                break
            elif stage_config.on_failure == "continue":
                stage_logger.info(f"  on_failure=continue, treating as non-critical")
                break

        # All retries exhausted
        stage_logger.error(f"\n  ❌ {stage_name} FAILED after {max_retries + 1} attempts")

        # Capture lesson about failure
        if first_error:
            stage_logger.info(f"  Capturing lesson from failure...")
            lesson_id = self.knowledge.add_lesson(
                stage=stage_name,
                error_type=classify_error(first_error),
                error_pattern=extract_error_pattern(first_error),
            ...
            stage_logger.info(f"  💡 Saved failure lesson: {lesson_id}")
    ...
    def _run_validate_stage(self, story_id: str, story_file: str) -> Tuple[bool, Optional[str]]:
        ...
        stage_config = self.config.stages.get(stage_name)
        if not stage_config:
            self.logger.warn(f"  ⚠️  No config for {stage_name}, skipping")
            return True, None
        ...
        stage_logger.info(f"  Pre-validation: Checking story file...")
        stage_logger.info(f"  Story ID: {story_id}")
        stage_logger.info(f"  Story file: {story_file}")

        story_path = Path(story_file)
        is_valid, error_msg = validate_file_exists(story_path, must_be_file=True, must_be_readable=True)

        if not is_valid:
            stage_logger.error(f"  ❌ Story file validation failed: {error_msg}")
            # Capture lesson
            self.knowledge.add_lesson(
                stage=stage_name,
        ...
        stage_logger.info(f"  ✓ Story file exists and is readable")

        # Check file has minimum content
        try:
            content = safe_read(story_path)
            if len(content.strip()) < 100:
                stage_logger.error(f"  ❌ Story file is too short ({len(content)} chars)")
                return False, None
            stage_logger.info(f"  ✓ Story file has content ({len(content)} chars)")
        except Exception as e:
            stage_logger.error(f"  ❌ Error reading story file: {e}")
            return False, None
        ...
        for attempt in range(max_retries + 1):
            ...
            # Check success
            if task_result.success:
                stage_logger.info(f"  ✅ {stage_name} agent completed ({elapsed:.1f}s)")

                # Parse validation result from output
                stage_logger.info(f"  Parsing validation results...")
                is_valid, validation_msg = self._parse_validation_result(task_result.output)

                if not is_valid:
                    stage_logger.error(f"  ❌ Validation FAILED: {validation_msg}")
                    stage_logger.info(f"  Story has validation issues that must be fixed")
                    ...
                    # Treat as failure and retry if available
                    if attempt < max_retries and stage_config.on_failure == "fix_and_retry":
                        stage_logger.info(f"  Retrying validation...")
                        first_error = validation_msg
                        first_error_details = task_result.output
                        continue
                    else:
                        stage_logger.error(f"  No more retries, validation failed")
                        return False, task_result.output

                stage_logger.info(f"  ✅ Validation PASSED: {validation_msg}")

                # Health check: Verify story still has all required sections
                stage_logger.info(f"  Running post-validation health checks...")
                health_passed, health_msg = self._verify_story_valid(story_file)

                if not health_passed:
                    stage_logger.warn(f"  ⚠️  Health check FAILED: {health_msg}")
                    # Non-critical, just warn
                    stage_logger.warn(f"  Warning: Story may have issues but validation passed")
            ...
            # Agent failed
            stage_logger.error(f"  ❌ {stage_name} agent FAILED ({elapsed:.1f}s)")
            stage_logger.info(f"  Exit code: {task_result.exit_code}")
    ...
    def _parse_validation_result(self, agent_output: Optional[str]) -> Tuple[bool, str]:
        ...
        self.logger.warn(f"  ⚠️  No clear PASS/FAIL in output, defaulting to PASS")
    ...
    def _verify_story_valid(self, story_file: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            self.logger.info(f"  ✓ Story file validated")
            self.logger.info(f"  ✓ All required sections present")
            self.logger.info(f"  ✓ Has tasks and acceptance criteria")
    ...
    def _verify_story_created(self, agent_output: Optional[str], expected_story_id: Optional[str] = None) -> Tuple[bool, str]:
        ...
        if expected_story_id and extracted_id != expected_story_id:
            ...
            self.logger.warn(f"  ⚠️  Story ID mismatch: expected {expected_story_id}, got {extracted_id}")
        ...
        try:
            ...
            self.logger.info(f"  ✓ Story file verified: {story_file.name}")
            self.logger.info(f"  ✓ File size: {len(content)} chars")
            self.logger.info(f"  ✓ All required sections present")
    ...
    def _run_spawn_stage(self, stage_name: str, **kwargs) -> bool:
        ...
        stage_config = self.config.stages.get(stage_name)
        if not stage_config:
            stage_logger.info(f"  No config for stage {stage_name}, skipping")
            return True
        ...
        if lessons:
            stage_logger.info(f"  📚 Applying {len(lessons)} lesson(s) from previous runs")
        ...
        for attempt in range(max_retries + 1):
            stage_logger.info(f"  Spawning {stage_name} agent (attempt {attempt + 1}/{max_retries + 1})...")
            ...
            if task_result.success:
                stage_logger.info(f"  {stage_name} PASSED ({task_result.duration_seconds:.1f}s)")
            ...
            stage_logger.info(f"  {stage_name} FAILED")
            ...
            # Retry if available
            if attempt < max_retries and stage_config.on_failure == "fix_and_retry":
                stage_logger.error(f"  Attempt {attempt + 1} failed, will retry...")
                continue

        # All retries exhausted
        if stage_config.on_failure == "continue":
            stage_logger.error(f"  {stage_name} failed but continuing (on_failure=continue)")
            return False

        stage_logger.error(f"  {stage_name} failed after {max_retries + 1} attempts")
        return False
    ...
    def _run_command_stage(self, stage_name: str) -> bool:
        ...
        """
        stage_logger = self._get_stage_logger(stage_name, self.story_id)
        ...
        if not command:
            stage_logger.info(f"  No command for stage {stage_name}, skipping")
            return True
        ...
        if lessons:
            stage_logger.info(f"  📚 {len(lessons)} lesson(s) available for {stage_name}")

        first_error = None

        for attempt in range(max_retries + 1):
            stage_logger.info(f"  Running: {command} (attempt {attempt + 1}/{max_retries + 1})...")
            ...
            try:
                ...
                if proc.returncode == 0:
                    stage_logger.info(f"  {stage_name} PASSED")
                ...
                stage_logger.info(f"  {stage_name} FAILED (exit code {proc.returncode})")
                ...
                if attempt < max_retries and stage_config.on_failure == "fix_and_retry":
                    stage_logger.info(f"  Spawning fix agent for {stage_name}...")
                    stage_logger.info(f"  Error preview: {errors[:300]}...")
                    ...
                    fix_result = self._wait_for_task(fix_task, f"{stage_name}-fix", stage_logger=stage_logger)

                    if fix_result.success:
                        stage_logger.info(f"  Fix agent completed, will retry {stage_name}...")
                    else:
                        stage_logger.error(f"  Fix agent failed: {fix_result.error[:200] if fix_result.error else 'unknown'}...")

                    continue

            except subprocess.TimeoutExpired:
                stage_logger.info(f"  {stage_name} timed out after {timeout}s")
    ...
    def _story_requires_tests(self, story_id: str, story_file: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            self.logger.warn(f"  ⚠ Error checking story type: {e}, defaulting to require tests")
        ...
        try:
            ...
            try:
                ...
                if not has_test_data:
                    self.logger.warn(f"  ⚠ Warning: TDM file missing expected test case structure")

            except yaml.YAMLError as e:
                self.logger.warn(f"  ⚠ Warning: TDM file is not valid YAML: {e}")
    ...
    def _pre_validate_test_case_generation(self, story_id: str, story_file: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if "## Acceptance Criteria" not in content and "## Success Criteria" not in content:
                self.logger.warn(f"  ⚠ Warning: Story file missing Acceptance Criteria section")
    ...
    def _verify_task_completed(self, story_file: str, task_index: int) -> Tuple[bool, str]:
        ...
        try:
            ...
            if "## File List" not in content and "## Files Changed" not in content:
                self.logger.warn(f"  ⚠ Warning: No File List section found in story file")

            # Check 4: Verify Change Log section exists (agent should update it)
            if "## Change Log" not in content and "## Changes" not in content:
                self.logger.warn(f"  ⚠ Warning: No Change Log section found in story file")
    ...
    def _run_develop_with_decomposition(self, story_id: str, story_file: str) -> bool:
        ...
        if not is_valid:
            stage_logger.error(f"  ❌ Pre-validation failed: {validation_msg}")
            stage_logger.info(f"")
            # Capture lesson
            self.knowledge.add_lesson(
                stage=stage_name,
        ...
        stage_logger.info(f"  ✅ Pre-validation passed: {validation_msg}")
        stage_logger.info(f"")
        ...
        try:
            ...
            if not tasks:
                stage_logger.warn("  ⚠ No tasks found in story file")
                stage_logger.info(f"")
                return True

            stage_logger.info(f"  ✅ Found {len(tasks)} task(s) in story")
            stage_logger.info(f"  Summary: {format_tasks_summary(tasks)}")
            stage_logger.info(f"")

            # Step 2: Check if decomposition needed
            if not should_decompose(tasks):
                stage_logger.info("  ✅ All tasks already complete, skipping development")
                stage_logger.info(f"")
                return True
            ...
            for i, task in enumerate(incomplete_tasks, 1):
                ...
                if lessons:
                    stage_logger.info(f"  📚 Applying {len(lessons)} lesson(s) from knowledge base")
                stage_logger.info(f"")
                ...
                for attempt in range(max_retries + 1):
                    if attempt > 0:
                        stage_logger.info(f"  🔄 Retry {attempt}/{max_retries} for task {task.index}")
                        stage_logger.info(f"")

                    # Spawn agent for THIS TASK ONLY
                    stage_logger.info(f"  🚀 Spawning agent for task {task.index}...")
                    ...
                    task_result = self._wait_for_task(task_bg, f"Task {task.index}", stage_logger=stage_logger)

                    if task_result.success:
                        stage_logger.info(f"  ✅ Agent completed (duration: {task_result.duration_seconds:.1f}s)")
                        ...
                        if not health_passed:
                            stage_logger.error(f"  ❌ Health check failed: {health_msg}")
                            stage_logger.info(f"")
                            ...
                            if attempt < max_retries:
                                ...
                                stage_logger.error(f"  ❌ Task {task.index} failed health check after {max_retries + 1} attempts")
                                break
                        else:
                            stage_logger.info(f"  ✅ Health check passed: {health_msg}")
                        ...
                        stage_logger.error(f"  ❌ Agent failed (attempt {attempt + 1}/{max_retries + 1})")
                        if task_result.error:
                        ...
                        if attempt < max_retries:
                            ...
                            stage_logger.error(f"  ❌ Task {task.index} failed after {max_retries + 1} attempts")
                            stage_logger.info(f"")

                if not task_passed or not task_health_passed:
                    stage_logger.info(f"{'─'*60}")
                    stage_logger.error(f"  ❌ Development failed at task {task.index}")
                    stage_logger.info(f"{'─'*60}")
                    stage_logger.info(f"")
                    return False
            ...
            stage_logger.info(f"{'='*60}")
            stage_logger.info(f"  ✅ All {len(incomplete_tasks)} task(s) completed successfully")
            stage_logger.info(f"{'='*60}")
            stage_logger.info(f"")
            return True
            ...
            stage_logger.info(f"")
            stage_logger.error(f"  ❌ Task decomposition error: {e}")
            import traceback
            traceback.print_exc()
    ...
    def _run_test_case_generation_stage(self, story_id: str, story_file: str) -> Tuple[bool, Optional[str]]:
        ...
        stage_config = self.config.stages.get(stage_name)
        if not stage_config:
            stage_logger.warn(f"  ⚠ Stage config not found for {stage_name}")
            stage_logger.info(f"")
            return False, None
        ...
        for attempt in range(max_retries + 1):
            if attempt > 0:
                stage_logger.info(f"  🔄 Retry {attempt}/{max_retries}")
                stage_logger.info(f"")

            # Spawn agent for test case generation
            stage_logger.info(f"  🚀 Spawning test architect agent...")
            ...
            try:
                ...
                if task_result.success:
                    ...
                    if not health_passed:
                        ...
                        if attempt < max_retries:
                            ...
                            stage_logger.error(f"  ⚠ Test case generation failed after {max_retries + 1} attempts")
                            stage_logger.info(f"")
                        ...
                        stage_logger.info(f"")
                        stage_logger.info(f"{'='*60}")
                        stage_logger.info(f"  ✅ Test case generation completed")
                        stage_logger.info(f"{'='*60}")
                        stage_logger.info(f"")
                        return True, task_result.output
                ...
                stage_logger.error(f"  ❌ Exception during test case generation: {e}")
    ...
    def _run_lint_stage(self, story_id: str, story_file: str) -> Tuple[bool, Optional[str]]:
        ...
        for attempt in range(max_retries + 1):
            ...
            stage_logger.info(f"  🚀 Spawning lint agent...")
            ...
            try:
                ...
                if task_result.success:
                    ...
                    stage_logger.info(f"  ✅ Lint checks passed (duration: {task_result.duration_seconds:.1f}s)")
                    ...
                    stage_logger.info(f"")
                    stage_logger.info(f"{'='*60}")
                    stage_logger.info(f"  ✅ Lint stage completed")
                    stage_logger.info(f"{'='*60}")
                    ...
                    stage_logger.error(f"  ❌ Lint checks failed (attempt {attempt + 1}/{max_retries + 1})")
                    if task_result.error:
                        error_preview = task_result.error[:200].replace('\n', ' ')
                        stage_logger.info(f"  Error: {error_preview}...")
                ...
                stage_logger.error(f"  ❌ Exception during lint: {e}")
        ...
        # All retries exhausted
        stage_end_time = time.time()
        stage_logger.error(f"  ❌ Lint failed after {max_retries + 1} attempts")
        stage_logger.info(f"")
    ...
    def _run_typecheck_stage(self, story_id: str, story_file: str) -> Tuple[bool, Optional[str]]:
        ...
        for attempt in range(max_retries + 1):
            ...
            stage_logger.info(f"  🚀 Spawning typecheck agent...")
            ...
            try:
                ...
                if task_result.success:
                    stage_logger.info(f"  ✅ Type checks passed (duration: {task_result.duration_seconds:.1f}s)")
                    ...
                    stage_logger.info(f"")
                    stage_logger.info(f"{'='*60}")
                    stage_logger.info(f"  ✅ Typecheck stage completed")
                    stage_logger.info(f"{'='*60}")
                    stage_logger.info(f"")
                    return True, task_result.output

                else:
                    stage_logger.error(f"  ❌ Type checks failed (attempt {attempt + 1}/{max_retries + 1})")
                    if task_result.error:
                        error_preview = task_result.error[:200].replace('\n', ' ')
                        stage_logger.info(f"  Error: {error_preview}...")
                ...
                stage_logger.error(f"  ❌ Exception during typecheck: {e}")
        ...
        # All retries exhausted
        stage_logger.error(f"  ❌ Typecheck failed after {max_retries + 1} attempts")
        stage_logger.info(f"")
        return False, None
    ...
    def _run_unit_test_stage(self, story_id: str, story_file: str) -> Tuple[bool, Optional[str]]:
        ...
        for attempt in range(max_retries + 1):
            ...
            stage_logger.info(f"  🚀 Spawning unit test agent...")
            ...
            try:
                ...
                if task_result.success:
                    stage_logger.info(f"  ✅ All unit tests passed (duration: {task_result.duration_seconds:.1f}s)")
                    ...
                    stage_logger.info(f"")
                    stage_logger.info(f"{'='*60}")
                    stage_logger.info(f"  ✅ Unit test stage completed")
                    stage_logger.info(f"{'='*60}")
                    stage_logger.info(f"")
                    return True, task_result.output

                else:
                    stage_logger.error(f"  ❌ Unit tests failed (attempt {attempt + 1}/{max_retries + 1})")
                    if task_result.error:
                        error_preview = task_result.error[:200].replace('\n', ' ')
                        stage_logger.info(f"  Error: {error_preview}...")
                ...
                stage_logger.error(f"  ❌ Exception during unit tests: {e}")
        ...
        # All retries exhausted
        stage_logger.error(f"  ❌ Unit tests failed after {max_retries + 1} attempts")
        stage_logger.info(f"")
        return False, None
    ...
    def _run_deploy_local_stage(self, story_id: str, story_file: str) -> Tuple[bool, Optional[str]]:
        ...
        stage_logger = self._get_stage_logger(stage_name, story_id)
        stage_logger.info(f"\n{'='*60}")
        stage_logger.info(f"  STAGE: Deploy Local")
        stage_logger.info(f"{'='*60}")
        stage_logger.info(f"  Story: {story_id}")
        stage_logger.info(f"")
        ...
        if not requires_deploy:
            stage_logger.warn(f"  ⚠ Skipping deployment: {reason}")
            stage_logger.info(f"")
        ...
        for attempt in range(max_retries + 1):
            ...
            stage_logger.info(f"  🚀 Spawning deployment agent...")
            ...
            try:
                ...
                if task_result.success:
                    stage_logger.info(f"  ✅ Deployment completed (duration: {task_result.duration_seconds:.1f}s)")
                    stage_logger.info(f"")
                    ...
                    if not health_passed:
                        ...
                        if attempt < max_retries:
                            ...
                            stage_logger.error(f"  ❌ Deployment failed health check after {max_retries + 1} attempts")
                            stage_logger.info(f"")
                            return False, None
                        ...
                        stage_logger.info(f"")
                        stage_logger.info(f"{'='*60}")
                        stage_logger.info(f"  ✅ Deploy local stage completed")
                        stage_logger.info(f"{'='*60}")
                        stage_logger.info(f"")
                        return True, task_result.output

                else:
                    stage_logger.error(f"  ❌ Deployment failed (attempt {attempt + 1}/{max_retries + 1})")
                    if task_result.error:
                        error_preview = task_result.error[:200].replace('\n', ' ')
                        stage_logger.info(f"  Error: {error_preview}...")
                ...
                stage_logger.error(f"  ❌ Exception during deployment: {e}")
        ...
        # All retries exhausted
        stage_logger.error(f"  ❌ Deployment failed after {max_retries + 1} attempts")
        stage_logger.info(f"")
        return False, None
    ...
    def _run_code_review_stage(self, story_id: str, story_file: str, files_changed: list) -> Tuple[bool, Optional[str]]:
        ...
        if not stage_config or not stage_config.enabled:
            stage_logger.warn(f"  ⚠ Code review stage not enabled")
            return True, "SKIP"
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                stage_logger.info(f"  🚀 Spawning code review agent...")
                stage_logger.info(f"")
                ...
                if task_result.success:
                    stage_logger.info(f"  ✅ Code review completed (duration: {task_result.duration_seconds:.1f}s)")
                    stage_logger.info(f"")
                    ...
                    if "no issues" in output_lower or "looks good" in output_lower or "approved" in output_lower:
                        issues_found = False
                        stage_logger.info(f"  ✅ Code review PASSED: No issues found")
                        ...
                        if issue_matches:
                            ...
                            issues_found = True
                            stage_logger.warn(f"  ⚠ Code review found {issue_count} issue(s)")
                        else:
                            issues_found = True
                            stage_logger.warn(f"  ⚠ Code review found issues (count unknown)")
                    else:
                        # Neutral result
                        stage_logger.info(f"  ✅ Code review completed (no clear pass/fail)")
                    ...
                    stage_logger.info(f"  ✅ Story status updated to 'review'")
                    stage_logger.info(f"")
                    ...
                    stage_logger.info(f"{'='*60}")
                    stage_logger.info(f"  ✅ Code review stage completed")
                    stage_logger.info(f"{'='*60}")
                    stage_logger.info(f"")
                    ...
                    stage_logger.error(f"  ❌ Code review failed (attempt {attempt + 1}/{max_retries + 1})")
                    if task_result.error:
                        error_preview = task_result.error[:200].replace('\n', ' ')
                        stage_logger.info(f"  Error: {error_preview}...")
                    stage_logger.info(f"")
                ...
                stage_logger.error(f"  ❌ Exception during code review: {e}")
                stage_logger.info(f"")
        ...
        stage_logger.error(f"  ⚠ Code review failed after {max_retries + 1} attempts (non-blocking)")
        stage_logger.info(f"")
        return True, "FAIL"
    ...
    def _update_story_status(self, story_id: str, new_status: str) -> None:
        ...
        try:
            ...
            if not sprint_status_file:
                self.logger.warn(f"  ⚠ sprint-status.yaml not found, skipping status update")
                return
            ...
            self.logger.info(f"  Story status updated: {story_id}: {old_status} → {new_status}")

        except Exception as e:
            self.logger.warn(f"  ⚠ Could not update story status: {e}")
    ...
    def _log_stage_execution(
        ...
        try:
            ...
            self.logger.info(f"  📝 Stage log written to: {log_file.relative_to(self.project_root)}")

        except Exception as e:
            self.logger.warn(f"  ⚠ Could not write stage log: {e}")
    ...
    def _wait_for_task(
        ...
        stage_logger: Optional[Logger] = None,
    ) -> TaskResult:
        """
        Wait for a background task with progress display.

        Args:
            task: Background task to wait for
            stage_name: Name of the stage (for logging)
            poll_interval: How often to check task status
            stage_logger: Optional stage-specific logger (if provided, agent output will be written to stage log)
        """
        logger = stage_logger if stage_logger else self.logger
        logger.info(f"  Waiting for {stage_name}...")
        ...
        while not task.is_done():
            ...
            if time.time() - last_log >= 1:
                logger.info(f"    ... {stage_name} still running ({elapsed:.0f}s)")
                last_log = time.time()

        result = task.get_result(block=False)
        logger.info(f"  {stage_name} completed: success={result.success}, duration={result.duration_seconds:.1f}s")

        if not result.success and result.error:
            error_preview = result.error[:500] if result.error else ""
            logger.info(f"    Error: {error_preview}...")

        # Write full agent output to stage log if stage_logger provided
        if stage_logger and result.output:
            logger.info(f"\n{'='*60}")
            logger.info(f"  Agent Output (Full)")
            logger.info(f"{'='*60}")
